...
expert_code <- cmdstan_model("estimate_beta_priors.stan")
...
over_states <- parallel::mclapply(unique(state_strat$state), function(s) {
  ...
  draws_df <- as_draws_df(expert_mod$draws(c("successes",
                                             "denominator",
                                             "failures")))
  
  tibble(state_id=s,
         inf_pr=mean(draws_df$successes/draws_df$denominator),
         ...
         survey_size=floor(mean(draws_df$denominator)),
         date_begin=ymd("2020-03-21"),
         date_end=ymd("2020-03-23"),
         alpha=mean(draws_df$successes),
         beta=mean(draws_df$failures)) %>% 
...
saveRDS(serology_real,"serology_real.rds")
...
real_data <- list(time_all=length(unique(combined$month_day)),
                 ...
                 RE=nrow(serology_real), # number of expert surveys (states)
                 S=ncol(covs),
                 G=ncol(covs_mob),
                 L=ncol(covs_lock),
...
if(run_model) {
  pan_model_scale <- cmdstan_model("corona_tscs_betab_mix_prior.stan",
   ...
   us_fit_scale_mod <- pan_model_scale$sample(data=real_data,
                           threads_per_chain = 4,
                           init=init_vals,iter_warmup = 500,
                           iter_sampling = 1500,
                           max_treedepth = 16,
                           parallel_chains=4)
