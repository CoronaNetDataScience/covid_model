...
functions {
  ...
  real partial_sum(int[,] y_slice,
    ...
    for(r in 1:size(y_slice)) {
        ...
        for(g in 1:G) {
          mu_mob[g] = mob_alpha_const[g] + Q_lock[start2:end2,1:L]*suppress_hier_const_raw[g]; 
        }
          log_prob += multi_normal_cholesky_lpdf(mob_array[1:G,start2:end2]|mu_mob,M_Sigma);
...
data {
  ...
  matrix[num_rows,1] fear; // COVID poll
...
transformed data {
  ...
  // need an array for mobility data for MVN Normal
